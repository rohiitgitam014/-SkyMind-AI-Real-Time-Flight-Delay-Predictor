...
if st.button("📡 Fetch & Analyze Flights"):
    ...
    if "delay" not in full_df.columns:
        full_df["delay"] = (full_df["velocity"] >= 100) & (full_df["geo_altitude"] > 1000)
    full_df = full_df.dropna(subset=["delay"])

    X = full_df[["velocity", "geo_altitude"]]
    y = full_df["delay"]

    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42, startify = True)
    ...
    st.write(f"### Model accuracy on historical data: {recallscore:.2%}")
    ...
    latest_df["Predicted Delay"] = latest_df["Predicted Delay"].apply(lambda x : "Not Deplay" if x ==  1 else "Deplay")
